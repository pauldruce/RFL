...
```text
D = sum_{i=1}^p (gamma^i (x) H_i) + sum_{j=1}^q (gamma^(p+j) (x) L_j)
```

As the research program expands into **Fermion Functional Integrals (Barrett 2024)**, **Product Geometries S_F^2 (x) F (Barrett 2026)**, and **Random Matrix Spectral Statistics**, the legacy codebase exhibits structural bottlenecks:
...
2. **Entangled Physics & Sampling:** The analytical trace variation formulas `Delta Tr(D^2)` and `Delta Tr(D^4)` are embedded directly inside `Metropolis.cpp`. This prevents reusing the same physics for Hybrid Monte Carlo (HMC) or new multi-term action potentials (`S_6`, `Pfaffian`).
...
### 1.2 Goals
* **Research-Centric Workflow:** Provide a non-blocking **Stepper & Observer API** that allows researchers in C++ and Python to step through Markov sweeps, stream eigenvalue spectra, and measure integrated autocorrelation times `tau_int`.
...
### 1.3 Non-Goals
* Distributed MPI cluster scaling (single-node multi-threading via OpenMP is sufficient for current matrix sizes N <= 256).
...
2. **Scenario 2 (Automated Batch Sampling & Statistics):** An automated batch script executes 100,000 production sweeps across a parameter grid `(g_2, g_4)`, recording eigenvalue spectra every 10 sweeps to compute the spectral dimension `d_spec` and edge eigenvalue statistics (e.g. comparing largest eigenvalue fluctuations against the [Tracy–Widom distribution](https://en.wikipedia.org/wiki/Tracy%E2%80%93Widom_distribution)).
3. **Scenario 3 (Extending Physics Actions):** A theorist implements a new action potential `S(D) = S_BG(D) - ln Pf(JD)` by subclassing or providing a new Action policy without modifying any Monte Carlo sampler code.
...
| **REQ-002** | **Exact Analytic Variation Delta S** | Fast `O(N^3)` local variation must equal global recomputation `S(D + delta D) - S(D)` within floating-point tolerance (< 10^-10). |
...
| **REQ-004** | **Detailed Balance & Ergodicity** | The MCMC stepper must satisfy detailed balance and provide an integrated autocorrelation estimator `tau_int` to compute rigorous observable errors. |
| **REQ-005** | **Automated Step-Size Calibration** | Dual-averaging sweeps must tune the proposal scale to achieve target acceptance rates (e.g. 0.65 ± 0.03) during burn-in. |
...
| **REQ-007** | **Active LTS Platform Policy** | Maintain compatibility with active Ubuntu LTS (≥ 22.04), macOS (≥ 14), Armadillo (≥ 11.4), and conforming C++17 compilers. |
...
| :--- | :--- | :--- | :--- |
| **Separation of Concerns**| ❌ Physics tangled in MCMC | ✅ **Clean separation (S, Delta S, grad S)** | ✅ Clean separation |
| **Computational Speed** | ✅ Fast analytical O(N^3) | ✅ **Fast analytical O(N^3)** | ❌ 10–50x slower for matrix traces |
| **Reusability for HMC** | ❌ Impossible without duplication | ✅ **Directly reuses gradient grad S** | ✅ Automatic gradients |
| **Decision** | Rejected | **Selected (Option B)** | Rejected |
...
```text
Z = integral dD exp(-S_B(D)) * Pfaffian(JD) = integral dD exp(-S_eff(D))
```

where the acceptance probability factorizes as:

```text
alpha = min(1, exp(-Delta S_B) * |Pfaffian(JD') / Pfaffian(JD)|)
```
...
| **Numerical Stability for Fermions** | ✅ High (Computes Pfaffian ratio directly without log-divergences) | ⚠️ Moderate (Requires -ln|Pf| additive energy conversion) |
| **Mathematical Alignment** | ✅ Matches exact factorization of path integral Delta S_B + ratio | ⚠️ Treats non-local Pfaffians as standard polynomial traces |
