...
TEST(CliffordTests, GammasHaveCorrectDims) {
  ...
  for (const auto &d : data) {
	...
	auto gammas = C.get_gammas();
	for (auto &g : gammas) {
	  ASSERT_EQ(g.n_rows, g.n_cols) << "Gamma matrices not squarefailed for (p,q) = (" << d.p << "," << d.q << ")";
...
TEST(CliffordTests, ChiralityIsCorrect) {
  ...
  for (auto &d : data) {
	...
	arma::cx_mat
		expect_chirality = gammas_producted * (std::complex<double>)std::pow(std::complex<double>(0, 1), exponent);
